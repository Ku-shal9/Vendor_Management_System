  ...
  return (
    <div className="vms-page">
      ...
      <div className="vms-panel overflow-hidden">
        {filteredPurchases.length === 0 ? (
          <EmptyState
            ...
            description="Create a purchase request when CLance Solutions needs items from an approved vendor."
          />
        ) : (
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const ok = await confirm({
                            title: "Delete",
                            message: `Delete ${prq.id}?`,
                            confirmLabel: "Delete",
                            destructive: true,
                          });
                          if (ok) onDeletePurchase(prq.id);
                        }}
                        className="p-2 text-ink-subtle hover:text-danger-ink"
                      >
      ...
      <Modal
        ...
        className="vms-panel p-6 max-w-lg w-full shadow-xl"
      >
        <h2
          ...
          {purchaseItemsCatalog.map((item) => (
            <div key={item.name} className="flex justify-between gap-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setPurchaseQuantities((p) => ({
                    ...p,
                    [item.name]: e.target.value.replace(/[^0-9]/g, ""),
                  }))
                }
                className="w-16 vms-input text-center"
              />
            </div>
          ))}
